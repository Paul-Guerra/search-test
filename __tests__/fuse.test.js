...
describe('Fuse', () => {
  ...
  let messages = [
    ...
    {
      id: 1,
      text: 'I learned a long time ago that worrying is like a rocking chair. It gives you something to do but it doesn’t get you anywhere.'
    },
    {
      id: 2,
    ...
    {
      id: 3,
      text: 'We get the warhead and we hold the world ransom for... One million dollars.'
    }
  ];
  const options = {
    includeScore: true,
    threshold: 0.6,
    location: 0,
    distance: 4000, // estimated length of longest message in products wildest dreams
    maxPatternLength: 32,
    minMatchCharLength: 1,
    keys: ['text']
  };
  let fuse = new Fuse(messages, options);

  it('finds matches inside of words', () => {
    let results = fuse.search('anno');
    expect(results.length).toBe(1);
  });

  it('finds matches across words', () => {
    let results = fuse.search('ave the gu');
    expect(results.length).toBe(1);
  });

  it('finds matches across all items', () => {
    let results = fuse.search('the');
    expect(results.length).toBe(3);
  });

  it('finds matches at the beginning, middle and end of words across items', () => {
    let results = fuse.search('li');
    expect(results.length).toBe(3);
  });

  it('single match in a large entry', () => {
    ...
    In ac felis quis tortor malesuada pretium. Pellentesque auctor neque nec urna. Proin sapien ipsum, porta a, auctor quis, euismod ut, mi. Aenean viverra rhoncus pede. Pellentesque habitant morbi tristique senectus et netus et malesuada fames ac turpis egestas. Ut non enim eleifend felis pretium feugiat. Vivamus quis mi. Phasellus a est. PAUL`;
    ...
    let fuse = new Fuse([{id: 0, text: bigText}], options);
    let results = fuse.search('PAUL');
    expect(results.length).toBe(1);
  });
});
