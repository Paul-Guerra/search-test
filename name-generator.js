...
module.exports = function makeRandomName() {
  let lindex = Math.floor(Math.random() * (left.length));
  let rindex = Math.floor(Math.random() * (right.length));
  let adj = `${left[lindex].slice(0, 1).toUpperCase()}${left[lindex].slice(1)}`;
  let surname = `${right[rindex].slice(0, 1).toUpperCase()}${right[rindex].slice(1)}`;
  return { first: adj, last: surname };
}
